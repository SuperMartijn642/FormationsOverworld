 ...
 */
public class SmallTempleStructure extends StructureConfigurator {

    public SmallTempleStructure(){
        super("small_temple");
    }
    ...
    protected void configureStructure(StructureConfiguration configuration){
        ...
        configuration.biomeTags(BiomeTags.IS_JUNGLE.location(), BiomeTags.IS_SAVANNA.location())
            ...
            .simpleType(properties -> properties.templatePool("small_temples").placement(StructurePlacement.SURFACE));
    }

    @Override
    protected void createTemplatePools(TemplatePoolGenerator generator){
        generator.pool("small_temples")
            .commonEntries(entry -> entry.groundLevel(1).processors(new BiomeReplacementProcessor(), new FormationsBlockAgeProcessor(0.3f, 0, 0)), "small_temple1", "small_temple2")
            .commonEntries(entry -> entry.groundLevel(1).processors(new BiomeReplacementProcessor(), new FormationsBlockAgeProcessor()), "small_temple1", "small_temple2");
    }
}
